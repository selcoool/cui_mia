...
float uncertaintyRoll = 2;
float t = 0.02; // 20ms loop time
...
void kalman_1d(float& KalmanState, float& KalmanUncertainty, float KalmanInput, float KalmanMeasurement) {
  ...
  KalmanUncertainty = KalmanUncertainty + (t * t * 4 * 4);  // noise variance = 4 deg/s^2
  float KalmanGain = KalmanUncertainty / (KalmanUncertainty + 9); // measurement noise = 3^2
...
void loop() {
  ...
  // Kalman filter áp dụng
  kalman_1d(kalmanPitch, uncertaintyPitch, gx / 131.0, rawPitch); // gx độ/giây
