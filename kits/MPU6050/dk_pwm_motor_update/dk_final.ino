        ...
        previousErrorRoll = errorRoll;
            ...
            if (joy1X == 4095 || joy1X == 0 || joy1Y == 0 || joy1Y == 4095 || joy2X == 4095 || joy2X == 0 || joy2Y == 4095 || joy2Y == 0) {
                ...
                if (joy1Y == 0) {
                    ...
                    if (speedAdjustment < 128) {
                        speedAdjustment += 10;  // Tăng thêm 10
                        if (speedAdjustment > 128) {
                            speedAdjustment = 128;  // Giới hạn giá trị không vượt quá 255
                        }
                    }
                  
                    Serial.println("Cập nhật biến: " + String(speedAdjustment));
                }
                // Khi joystick 1 ở vị trí cực tiểu (0)
                else if (joy1Y == 4095) {
                    // Giảm giá trị, nhưng không dưới 0
                    if (speedAdjustment > 0) {
                        speedAdjustment -= 10;  // Giảm đi 10
                        if (speedAdjustment < 0) {
                            speedAdjustment = 0;  // Giới hạn giá trị không nhỏ hơn 0
                        }
                    }
                    // Điều chỉnh tốc độ động cơ dựa trên kết quả PID
                    motorSpeed1 = constrain(128 + pidOutputPitch + pidOutputRoll + speedAdjustment, 0, 255);
                    motorSpeed2 = constrain(128 - pidOutputPitch + pidOutputRoll + speedAdjustment , 0, 255);
                    motorSpeed3 = constrain(128 + pidOutputPitch - pidOutputRoll + speedAdjustment, 0, 255);
                    motorSpeed4 = constrain(128 - pidOutputPitch - pidOutputRoll + speedAdjustment, 0, 255);
                    Serial.println("Cập nhật biến: " + String(speedAdjustment));
                }
                 else if (joy1X == 0) {
                    // Giảm giá trị, nhưng không dưới 0
                    if (speedAdjustment > 0) {
                        speedAdjustment -= 5;  // Giảm đi 10
                        if (speedAdjustment < 0) {
                            speedAdjustment = 0;  // Giới hạn giá trị không nhỏ hơn 0
                        }
                    }
                    ...
                    // Giảm giá trị, nhưng không dưới 0
                    if (speedAdjustment < 128) {
                        speedAdjustment += 5;  // Tăng thêm 10
                        if (speedAdjustment > 128 ) {
                            speedAdjustment = 128;  // Giới hạn giá trị không vượt quá 255
                        }
                    }
                    ...
                    Serial.println("Cập nhật biến: " + String("P"));
                }else if (joy2X == 4095) {
                   
            
                      Serial.println("Cập nhật biến: " + String("T"));
                }
                  else if (joy2Y == 0) {
                    
                  
                    Serial.println("Cập nhật biến: " + String("PT"));
                }else if (joy2Y == 4095) {
                   
            
                      Serial.println("Cập nhật biến: " + String("PS"));
                }
            }else{

                      // Điều chỉnh tốc độ động cơ dựa trên kết quả PID
         motorSpeed1 = constrain(128 + pidOutputPitch + pidOutputRoll + speedAdjustment + sideSpeedAdjustment, 0, 255);
      motorSpeed2 = constrain(128 - pidOutputPitch + pidOutputRoll + speedAdjustment + sideSpeedAdjustment , 0, 255);
      motorSpeed3 = constrain(128 + pidOutputPitch - pidOutputRoll + speedAdjustment + sideSpeedAdjustment, 0, 255);
     motorSpeed4 = constrain(128 - pidOutputPitch - pidOutputRoll + speedAdjustment + sideSpeedAdjustment, 0, 255);
            }
