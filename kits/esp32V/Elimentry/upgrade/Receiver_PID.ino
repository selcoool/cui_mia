...
void loop() {
  ...
  if (newPacket) {
    ...
    // Map transmitter data to angles
    rollSet  = ((float)rxData.joyX / 2048.0) * 15.0;     // -15 → 15
    pitchSet = ((float)rxData.joyY / 2048.0) * 15.0;    // -15 → 15
    yawSet   = ((float)rxData.joyZ / 2048.0) * 10.0;    // -10 → 10

    // Map throttle
    baseThrottle = map(rxData.throttle, -2048, 2048, 0, pwmMax);
  ...
  if (mpuReady) {
    ...
    // --- Tính Roll/Pitch từ gia tốc ---
    rollInput  = atan2(ay, az) * 57.3;    // độ
    pitchInput = atan2(-ax, az) * 57.3;   // độ
    ...
    yawOutput   = computePID(yawSet, 0, yawIntegral, yawErrorPrev); // yawInput = 0 vì lấy từ transmitter trực tiếp

    // --- MOTOR MIX ---
    motorSpeed[0] = baseThrottle + rollOutput - pitchOutput + yawOutput;
    motorSpeed[1] = baseThrottle - rollOutput - pitchOutput - yawOutput;
    motorSpeed[2] = baseThrottle - rollOutput + pitchOutput + yawOutput;
    motorSpeed[3] = baseThrottle + rollOutput + pitchOutput - yawOutput;
    ...
    Serial.print(" Yaw="); Serial.println(yawSet);  // hiển thị yaw đã map
  }
}
