...
PID pidR, pidP, pidY;
float lastThrottle = 0;   // 🔥 thêm dòng này
...
void loadPin(){
  ...
  M2_PIN=prefs.getInt("m2",6);
  M3_PIN=prefs.getInt("m3",5);
  M4_PIN=prefs.getInt("m4",4);
  prefs.end();
}
...
void loop(){
  ...
  // CH5 bật auto landing
    if(rx.ch[5] > 1600){
      if(!autoLand){
        altTarget = altitude; // bắt đầu từ độ cao hiện tại
        autoLand = true;
      }
    }else{
      autoLand = false;
...
float altHold = 0;
  ...
  // // ===== chống rơi tự do khi giảm ga =====
  //   float maxDrop = 0.3;   // chỉnh 0.5 → 2.0

  //   if(throttle < lastThrottle){
  //     throttle = lastThrottle - maxDrop;
  //   }
...
// // ===== SOFT LANDING + ANTI DROP =====

// // tốc độ giảm chậm hơn
// float dropRate = 0.01;   // 🔥 giảm nữa để đáp mượt hơn (0.1 -> rất mềm)

// // giữ lực tối thiểu khi gần đất
// float minHold = PWM_MIN + 0.5;  // 🔥 chống tắt motor đột ngột

// if(throttle < lastThrottle){
//   throttle = lastThrottle - dropRate;
// }

// // clamp không cho tụt quá nhanh về 0
// if(throttle < minHold){
//   throttle = minHold;
// }
...
if(autoLand){
  ...
  // giảm target từ từ (đi xuống)
  altTarget -= 0.02;   // 🔥 tốc độ hạ (0.01 -> rất chậm)

  if(altTarget < 0) altTarget = 0;

  float err = altTarget - altitude;

  // PID đơn giản
  float k = 30;   // 🔥 chỉnh lực giữ

  float altOut = err * k;

  // giới hạn lực
  altOut = constrain(altOut, -40, 40);

  throttle = PWM_MIN + altOut;

  // gần đất → giảm lực mạnh để đáp mềm
  if(altitude < 0.5){
    throttle *= 0.7;
  }
  if(altitude < 0.2){
