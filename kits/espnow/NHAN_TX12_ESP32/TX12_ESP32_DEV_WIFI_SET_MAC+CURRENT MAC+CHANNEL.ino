...
void handleGetChannels(){
  uint16_t chCopy[CHANNELS];
  noInterrupts();
  for(int i=0;i<CHANNELS;i++) chCopy[i]=channels[i];
  interrupts();

  String json = "[";
  for(int i=0;i<CHANNELS;i++){
    json += String(chCopy[i]);
    if(i<CHANNELS-1) json += ",";
  }
  json += "]";
  server.send(200,"application/json", json);
}
