...
.then(cfg => {
  ...
  document.body.insertAdjacentHTML('beforeend', '<p>Chip MAC: '+cfg.chipMAC+'</p>');
});
</script>
...
String macToByteArray(uint8_t *mac){
  char buf[40];
...
void handleGetConfig(){
  ...
  json += "\"chipMAC\":\""+macToByteArray(chipMAC)+"\"}";
...
void loop(){
  ...
  if(millis()-lastSend>20){ // 50Hz
    ...
    // --- Hiển thị chi tiết các kênh trên Serial ---
