...
#define LIGHT_PIN 4 // Chân đọc giá trị từ Potentiometer

#define PWM_LIGHT_CHANNEL 3  // Kênh PWM
#define PWM_MOTOR_CHANNEL 2  // Kênh PWM
...
<body>
    ...
    <div class="slider-container">
        ...
        <input type="range" id="speedSlider" min="0" max="255" value="50">
    </div>
    <div class="slider-container">
        <p>🚨 Độ sáng: <span id="lightValue">50</span></p>
        <input type="range" id="lightSlider" min="0" max="255" value="50">
    </div>
    ...
    <script>
        ...
        buttons.forEach(btn => {
            btn.onclick = () => {
                if (isButtonDisabled) return; // Nếu đang chờ, bỏ qua sự kiện click
                ...
                if (socket.readyState === WebSocket.OPEN) {
                    ...
                    setTimeout(() => {
                        isButtonDisabled = false; // Kích hoạt lại sau 500ms
                    }, 300);
                } else {
        ...
        lightSlider.addEventListener("change", () => {
            if (socket.readyState === WebSocket.OPEN) {
                socket.send(`light:${lightSlider.value}`);
                console.log(`🚨 Đã gửi độ sáng: ${lightSlider.value}`);
            }
        });
    </script>
...
void setup() {
  ...
  ledcAttachPin(ENA, PWM_MOTOR_CHANNEL);

  ledcSetup(PWM_LIGHT_CHANNEL, PWM_FREQ, PWM_RESOLUTION);
...
void setSpeed(int valueSpeed){
ledcWrite(PWM_MOTOR_CHANNEL, valueSpeed);
}

void setLight(int valueLight){
ledcWrite(PWM_LIGHT_CHANNEL, valueLight);
}
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
    switch (type) {
        ...
        case WStype_TEXT: {
            ...
            else if (command.rfind("light:", 0) == 0) {  // Kiểm tra nếu chuỗi bắt đầu bằng "speed:"
                int light = std::stoi(command.substr(6));  // Lấy giá trị sau "speed:"
                setLight(light);

               Serial.println("light:");
                webSocket.sendTXT(num, ("Light Set: " + std::to_string(light)).c_str());
            }
