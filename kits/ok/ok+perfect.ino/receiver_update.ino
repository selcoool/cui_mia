...
unsigned long lastThrottleAdjust = 0;
...
void loop() {
  ...
  if (packetSize) {
    ...
    if (received.indexOf("joy1Y:") != -1 && received.indexOf("joy1X:") != -1) {
  ...
  // ==== Điều khiển ====
 // ==== Joystick Điều khiển ====
  float pitchTarget = map(joy1X, 0, 4095, -30, 30);
  float rollTarget  = map(joy1Y, 0, 4095, -30, 30);

  // Tăng giảm throttle
  int delta = map(joy2X, 0, 4095, -5, 5);
  if (millis() - lastThrottleAdjust > 100 && abs(joy2X - 2048) > 200) {
    currentThrottle += delta;
    currentThrottle = constrain(currentThrottle, 80, 230);
    lastThrottleAdjust = millis();
  }
